...
if(homeRole.innerHTML == "Teacher"){
  ...
  var getClassInfo = firebase.database().ref("Classes");
  ...
  var showRoomDB = database.firebase().ref().child("Classes");
  showRoomDB.on("child_changed", snap => {
    var classname = snap.child("TheClass").val();
